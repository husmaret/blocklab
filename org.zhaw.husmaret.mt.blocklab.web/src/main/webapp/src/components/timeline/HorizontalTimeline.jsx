...
import {cummulativeSeperation} from './helpers/helpers';
 ...
 */
const defaultGetLabel = (date, index) => (dayjs(date)).format('DD.MM.YYYY HH:mm');
...
class HorizontalTimeline extends React.Component {
  ...
  render() {
    ...
    var dates = this.props.timelineData.map((timelineEvent) => new Date(timelineEvent.date));

    var visibleWidth = this.props.containerWidth - 80;

    // Calculate the distances for all events
    var distances = cummulativeSeperation(
      dates,
      props.labelWidth,
    ...
    var events = distances.map((distance, index) => ({
      distance,
      tooltip: this.props.timelineData[index].title,
      label: props.getLabel(this.props.timelineData[index].date, index),
      date: this.props.timelineData[index].date,
      isTimeout: this.props.timelineData[index].isTimeout
    }));
