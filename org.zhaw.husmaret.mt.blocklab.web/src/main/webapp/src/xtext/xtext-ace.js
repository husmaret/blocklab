...
import EditorContext from './AceEditorContext';
	...
	exports.createEditor = function(options) {
        ...
        var editor = ace.edit(options.container);
        editor.$blockScrolling = Infinity;
        if (options.position)
            jQuery(options.container).css('position', options.position);
        
        var editorOptions = ServiceBuilder.mergeParentOptions(options.container, options);
	...
	AceServiceBuilder.prototype.setupSyntaxHighlighting = function() {
		...
		if (options.syntaxDefinition != 'none' && (options.syntaxDefinition
            //  || options.xtextLang
             )) {
			var syntaxDefinition = options.syntaxDefinition;
            // TODO uncomment again at some point
			// if (!syntaxDefinition)
			// 	syntaxDefinition = 'xtext-resources/mode-' + options.xtextLang;
			if (typeof(syntaxDefinition) === 'string') {
				ace.require([syntaxDefinition], function(mode) {
